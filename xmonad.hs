#!/usr/bin/env stack
-- stack --resolver lts-10.5 --install-ghc runghc --package xmonad xmonad-contrib
...
myTerminal :: String
myTerminal = "kitty" -- all looks perfect "alacritty" -- best support for colors
...
myAppOnStartup =
  flip (++) "&" . intercalate " &" $
  [ "compton"
  , "dunst -conf ~/.xmonad/dunstrc" -- REMEMBER: GPMDP: needs notifications system up
  , "stalonetray --window-strut none --sticky -t --geometry 1x1+0+0 --grow-gravity W"
  , "nm-applet"
  , "nitrogen --restore"
  , "gnome-keyring-daemon --start --components=gpg,pkcs11,secrets,ssh"
  , "python ~/.xmonad/keyboard.py"
  , "pasystray"
  --, "taffybar"
  --, "lxqt-panel"
  --, "yabar"
...
myFadeHook =
  composeAll
    ...
    , isUnfocused --> opacity 0.90 -- all unfocused
...
ppXbar =
  ...
  , ppOrder =
    \(ws:l:t:[]) ->
       [ t
       , xmobarColor red black "\61762" --  "" " "-- "<fc=white,black>\57522</fc><fc=#a9212e,white>"
       , l
       , xmobarColor red black "\61762" --  "" -- "</fc><fc=black,white>\57522</fc>"
       , ws -- 
        ]
  , ppSep = " "
...
scratchpads =
  [ NS "terminal" "kitty --class=scratch" (className =? "scratch") (customFloating $ W.RationalRect 0 0 1 (15 / 50))
...
myManageHook =
  ...
  where
    ...
    myFloats = ["Terminator Preferences", "notification-daemon", "plasma-desktop", "klipper", "lxqt-panel", "Yakuake"]
    myIgnores = ["xfce4-notifyd", "conky", "gnome-panel", "oblogout", "plasmashell"]
    my1Shifts = []
...
myKeyBindings =
  ...
  , ("M-g", spawn "rofi -show window -theme ~/.xmonad/arthur.rasi") -- Switch between windows
  , ("M-q"
    , spawn $
      concat
        [ "killall dzen2 ;"
        ...
        , "killall compton ;"
        , "killall xmobar;"
        , "killall polychromatic-try-applet;"
        , "xmonad --recompile && xmonad --restart"])
  , ("M-f", spawn "rofi -combi-modi drun,run -show combi -theme ~/.xmonad/arthur.rasi") -- launcher
  ...
  , ("<XF86AudioMute>",        spawn "amixer -c 0 set Master toggle") -- Always a disaster make works
...
myplaneKeys ln limits =
  ...
  | (keySym, direction) <-
     zip ["<Left>", "<Down>", "<Right>", "<Up>", "a", "s", "d", "w"] $ cycle $ enumFrom ToLeft
